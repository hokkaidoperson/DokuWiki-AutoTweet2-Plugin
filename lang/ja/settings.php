...
$lang['template'] = 'メッセージのテンプレート<br>置換される文字列:<br>###WIKITITLE###→このWikiのタイトル<br>###PAGETITLE###→編集したページのタイトル<br>###TYPE###→編集のタイプ（「編集」「復元」「作成」「削除」「編集（小変更）」のいずれかが表示されます）<br>###SUMMARY###→編集の概要<br>###PAGEURL###→編集したページにアクセスするURL（###PAGEURL### が無い場合は、テンプレートの末尾にURLが自動付加されます）';
